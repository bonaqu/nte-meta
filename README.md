...
- Поиск и фильтры персонажей и гайдов по тиру, атрибуту, редкости и текстовым тегам; дублирующие фильтры роли и типа убраны.
...
На обычном push Actions запускает typecheck, lint, production build, проверку API и два коротких browser-smoke сценария: загрузку приложения и главную meta-страницу. Полный Playwright-набор запускается только вручную через `workflow_dispatch` с `full_suite=true`, например перед рискованным релизом. Локально достаточно focused-проверки затронутого сценария; полный e2e перед каждым небольшим изменением не требуется.
...
Автоимпорт ограничен 14 приоритетными источниками на запрос, четырьмя одновременными внешними соединениями и 600 KB ответа на источник. Остальные подключённые страницы остаются в списке для ручной проверки. Это удерживает запрос ниже лимитов Workers Free по subrequests/соединениям и не позволяет одному тяжёлому сайту заблокировать редактор.

Актуальная последняя миграция D1: `0023_fix_hotori_guide_markdown_newlines.sql`.
